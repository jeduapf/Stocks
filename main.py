...
def get_brazil_tickers():
    br_tickers = pd.read_csv("BR.csv")
    return list(br_tickers['Symbol'])

def main():
    
    inicio = datetime.datetime(2003,2,11)
    fim = datetime.datetime(2023,9,2)

    # name,stock,L = get_stock ('MGLU3.SA', inicio, fim)
    # plot_stock(name, stock, feature = 'Close')
    # # plot_stock(name, stock, feature = None)
    ...
    plot_stock('AFYA', BR_Stocks['AFYA'], feature = 'Close')
